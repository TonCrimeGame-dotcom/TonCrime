...
export class TradeScene {
  ...
  async _appendWalletLedger(entryType, ytonAmount = 0, tonAmount = 0, note = "", refId = null) {
    try {
      const profileId = await this._getProfileId();
      const telegramId = this._getTelegramId();
      const username = String(this.store.get()?.player?.username || "Player");
      ...
      const { error } = await supabase.from("wallet_ledger").insert([
        {
          ...
          username,
          entry_type: String(entryType || "reward"),
      ...
      console.warn("[TradeScene] wallet_ledger insert skipped:", err?.message || err);
    }
  }
  ...
  _doFreeSpin() {
    ...
    if (reward.type === "coins") {
      this._appendWalletLedger("reward", Number(reward.amount || 0), 0, `Premium Çark: ${reward.text}`);
    } else if (reward.type === "energy") {
      this._appendWalletLedger("reward", 0, 0, `Premium Çark: ${reward.text}`);
    } else {
      this._appendWalletLedger("reward", 0, 0, `Premium Çark item: ${reward.text}`);
    }
  ...
  _buyCrate(kind) {
    ...
    this._appendWalletLedger("purchase", -cost, 0, `${name} satın alındı`);
  ...
  _sellInventoryItem(itemId) {
    ...
    this._appendWalletLedger("sale", gain, 0, `${item.name || "Item"} NPC satış`);
