...
function randInt(min, max) { const a = Math.ceil(min); const b = Math.floor(max); return Math.floor(Math.random() * (b - a + 1)) + a; }
...
export class CoffeeShopScene {
  ...
  _ensureState() {
    ...
    const visitors = Array.isArray(cs.visitors) && cs.visitors.length ? cs.visitors : this._freshVisitors();
    ...
    this.store.set({
      ...
      missions: { ...missions, pvpPlayed: Number(missions.pvpPlayed || 0) },
      player: {
        ...p,
      ...
      coffeeshop: {
        ...
        theftEvents: Number(cs.theftEvents || 0),
  ...
  _freshVisitors() {
    return [...VISITORS].sort(() => Math.random() - 0.5).slice(0, 4).map((v) => ({
      ...v,
  ...
  _pushSystemChat(text) {
    ...
    const item = { id: makeId("sys"), type: "system", username: "SYSTEM", text: String(text || ""), ts: Date.now() };
    chatLog.push(item);
  ...
  _pushNotice(text) {
    ...
    notices.unshift({ id: makeId("notice"), text: String(text || ""), ts: Date.now() });
    this.store.set({ coffeeshop: { ...cs, notices: notices.slice(0, 3) } });
  }
  ...
  _giftPool() {
    return [
      { kind: "girls", icon: "💋", name: "VIP Girl", rarity: "epic", qty: 1, usable: true, sellPrice: 65, marketPrice: 92, energyGain: 22, desc: "Hediye kadın.", source: "coffeeshop_gift" },
      { kind: "consumable", icon: "🍾", name: "Premium Champagne", rarity: "rare", qty: 1, usable: true, sellPrice: 28, marketPrice: 44, energyGain: 14, desc: "Hediye alkol.", source: "coffeeshop_gift" },
      { kind: "goods", icon: "🌿", name: "Moon Rocks", rarity: "rare", qty: 1, usable: true, sellPrice: 22, marketPrice: 34, energyGain: 18, desc: "Hediye ot.", source: "coffeeshop_gift" },
      { kind: "rare", icon: "📦", name: "Mystery Crate", rarity: "legendary", qty: 1, usable: false, sellPrice: 55, marketPrice: 78, energyGain: 0, desc: "Hediye kasa.", source: "coffeeshop_gift" },
    ];
  }
  ...
  _maybeGift() {
    ...
    const s = this.store.get() || {};
    const cs = s.coffeeshop || {};
    this.store.set({ coffeeshop: { ...cs, giftsClaimed: Number(cs.giftsClaimed || 0) + 1 } });
  ...
  _maybeCrowdTheft(source = "crowd") {
    ...
    const p = s.player || {};
    const cs = s.coffeeshop || {};
    const energyLoss = Math.min(Number(p.energy || 0), randInt(4, 14));
    const coinLoss = Math.min(Number(s.coins || 0), randInt(6, 28));
    this.store.set({
      coins: Math.max(0, Number(s.coins || 0) - coinLoss),
      player: { ...p, energy: Math.max(0, Number(p.energy || 0) - energyLoss) },
      coffeeshop: { ...cs, securityHits: Number(cs.securityHits || 0) + 1, theftEvents: Number(cs.theftEvents || 0) + 1 },
    });
    this.blurUntil = Date.now() + 1800;
    this.flashUntil = Date.now() + 220;
    const text = `🥷 İçeride kavga çıktı (${source}) • -${energyLoss} enerji / -${coinLoss} yton`;
  ...
  _recordGlobalPvp(visitor, didWin, ratingDelta) {
    ...
    this.store.set({
      ...
      missions: { ...missions, pvpPlayed: Number(missions.pvpPlayed || 0) + 1 },
  ...
  _buy(item) {
    ...
    this.store.set({
      ...
      player: { ...p, coffeeshopUses: nextUses },
      coffeeshop: { ...cs, totalSpent: Number(cs.totalSpent || 0) + price, totalBought: Number(cs.totalBought || 0) + 1 },
    });
    ...
    if (energy >= energyMax) {
      ...
      this.store.set({ player: { ...this.store.get().player, energy: energy + gain, coffeeshopUses: nextUses } });
  ...
  _attack(visitor) {
    ...
    this.store.set({
      coins: nextCoins,
      player: { ...this.store.get().player, energy: nextEnergy },
      coffeeshop: { ...this.store.get().coffeeshop, visitors },
    });

    if (chance(0.18)) this._maybeCrowdTheft("pvp");
    ...
    if (this.dragging && this.input?.justReleased?.()) {
      ...
      for (const h of this.hitButtons) {
        ...
        if (h.action === "refresh") {
          const s = this.store.get() || {};
          const cs = s.coffeeshop || {};
          this.store.set({ coffeeshop: { ...cs, visitors: this._freshVisitors() } });
  ...
  render(ctx) {
    ...
    ctx.fillText(`Risk: ${fmtNum(cs.theftEvents || 0)} olay`, panelX + 290, panelY + 88);
