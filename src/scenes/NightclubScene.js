export class NightclubScene {
  ...
  render(ctx, w, h) {
    ...
    ctx.fillText("Enerji doldur, pazara koy, risk al.", panelX + 16, panelY + 48);
    ...
    ctx.fillText(`Enerji: ${energy}/${energyMax}`, panelX + panelW - 16, panelY + 42);

    ctx.fillStyle = "rgba(255,170,210,0.88)";
    ctx.fillText(`Pazar stoğu: ${invCount} • Baskın: ${raidCount}`, panelX + panelW - 16, panelY + 60);

    const topBtnY = panelY + 68;
    const backW = 88;
    const backH = 26;
    const pvpW = 110;
    const pvpH = 30;

    this.hitBack = { x: panelX + 12, y: topBtnY, w: backW, h: backH };
    this.hitPvp = { x: panelX + panelW - pvpW - 12, y: topBtnY - 2, w: pvpW, h: pvpH };

    this._roundRect(ctx, this.hitBack.x, this.hitBack.y, this.hitBack.w, this.hitBack.h, 10);
    ...
    ctx.font = "700 12px system-ui";
    ctx.fillText("← Geri", this.hitBack.x + this.hitBack.w / 2, this.hitBack.y + 17);

    this._roundRect(ctx, this.hitPvp.x, this.hitPvp.y, this.hitPvp.w, this.hitPvp.h, 11);
    ctx.fillStyle = "rgba(120,20,20,0.42)";
    ctx.fill();
    ctx.strokeStyle = "rgba(255,255,255,0.18)";
    ctx.stroke();
    ctx.fillText("⚔ PvP Saldırı", this.hitPvp.x + this.hitPvp.w / 2, this.hitPvp.y + 19);
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      ctx.font = "12px system-ui";
      ctx.fillText(`Sınıf: ${item.rarity}`, listX + 12, y + 41);
      ctx.fillText(`+${item.energy} enerji`, listX + 12, y + 59);

      ctx.fillStyle = "rgba(255,210,120,0.95)";
      ctx.fillText(`${item.price} yton`, listX + 126, y + 59);

      ctx.fillStyle = "rgba(255,170,210,0.90)";
      ctx.fillText(`Pazar: ${item.marketValue} yton`, listX + 210, y + 59);
