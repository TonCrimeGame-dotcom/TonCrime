...
const WEAPON_COPY = {
  glock_17: { tierEn: "Sidearm", labelEn: "Starter" },
  sig_p320: { tierEn: "Sidearm", labelEn: "Balanced" },
  beretta_92fs: { tierEn: "Sidearm", labelEn: "Stable" },
  colt_1911: { tierEn: "Sidearm", labelEn: "Heavy Hit" },
  mossberg_500: { tierEn: "Shotgun", labelEn: "Close Range" },
  rem_870: { tierEn: "Shotgun", labelEn: "Close Range" },
  mp5: { tierEn: "SMG", labelEn: "Fast" },
  ump45: { tierEn: "SMG", labelEn: "Controlled" },
  ar15: { tierEn: "Rifle", labelEn: "Mid Tier" },
  ak74: { tierEn: "Rifle", labelEn: "Mid Tier" },
  ak47: { tierEn: "Rifle", labelEn: "Powerful" },
  m4a1: { tierEn: "Rifle", labelEn: "Balanced" },
  g3: { tierEn: "Heavy Rifle", labelEn: "High Bonus" },
  scar_h: { tierEn: "Heavy Rifle", labelEn: "High Bonus" },
  svd: { tierEn: "Sniper", labelEn: "Premium" },
  m24: { tierEn: "Sniper", labelEn: "Premium" },
  m79: { tierEn: "Launcher", labelEn: "Epic" },
  rpg7: { tierEn: "Launcher", labelEn: "Epic" },
  barrett_m82: { tierEn: "Heavy", labelEn: "Legend" },
  m134: { tierEn: "Heavy", labelEn: "Legend" },
};
...
export class WeaponsScene {
  ...
  _weaponCopy(item) {
    return WEAPON_COPY[item?.id] || {};
  }

  _weaponTier(item) {
    const copy = this._weaponCopy(item);
    return this._lang() === "en" ? copy.tierEn || item?.tier || "" : item?.tier || "";
  }

  _weaponLabel(item) {
    const copy = this._weaponCopy(item);
    return this._lang() === "en" ? copy.labelEn || item?.label || "" : item?.label || "";
  }
  ...
  render(ctx, w, h) {
    ...
    ctx.fillText(this._ui("Silah Pazari", "Weapons Market"), headerRect.x + 14, headerRect.y + 24);
    ctx.fillStyle = "rgba(255,220,176,0.74)";
    ctx.font = `${isTiny ? 10 : 11}px system-ui`;
    ctx.fillText(
      this._ui("Silah sec, satin al ve kusan.", "Pick a weapon, buy it, and equip it."),
      headerRect.x + 14,
      headerRect.y + 42
    );
    ...
    const infoLeft = infoArtRect.x + infoArtRect.w + 14;
    const infoTextMaxW = unRect.x - infoLeft - 14;
    const equippedLabel = equipped ? equipped.name : this._ui("Silah secili degil", "No weapon equipped");
    const infoTitleSize = fitText(ctx, equippedLabel, infoTextMaxW, isTiny ? 14 : 15, 11, 900);
    const equippedSprite = equipped ? this._getWeaponSprite(equipped) : null;

    ctx.fillStyle = "rgba(255,255,255,0.06)";
    fillRoundRect(ctx, infoArtRect.x, infoArtRect.y, infoArtRect.w, infoArtRect.h, 14);
    ctx.strokeStyle = "rgba(255,194,96,0.16)";
    strokeRoundRect(ctx, infoArtRect.x + 0.5, infoArtRect.y + 0.5, infoArtRect.w - 1, infoArtRect.h - 1, 14);

    if (equippedSprite && (equippedSprite.complete || equippedSprite.naturalWidth)) {
      containImage(ctx, equippedSprite, infoArtRect.x + 8, infoArtRect.y + 8, infoArtRect.w - 16, infoArtRect.h - 16, 1);
    } else {
      ctx.fillStyle = "rgba(255,255,255,0.72)";
      ctx.font = `800 ${isTiny ? 9 : 10}px system-ui`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(equipped ? this._weaponTier(equipped) : "N/A", infoArtRect.x + infoArtRect.w / 2, infoArtRect.y + infoArtRect.h / 2);
    }
    ...
    ctx.font = `${isTiny ? 10 : 11}px system-ui`;
    ctx.fillText(
      this._ui(`Aktif bonus: +%${effectPct} - ikon ~${effectMs}ms`, `Active bonus: +%${effectPct} - icon ~${effectMs}ms`),
      infoLeft,
      infoRect.y + 44
    );
    ctx.fillText(
      equipped
        ? this._ui(`Sinif: ${this._weaponTier(equipped)} - ${this._weaponLabel(equipped)}`, `Tier: ${this._weaponTier(equipped)} - ${this._weaponLabel(equipped)}`)
        : this._ui(`Zaman carpani x${effectSlow.toFixed(2)}`, `Time scale x${effectSlow.toFixed(2)}`),
      infoLeft,
      infoRect.y + 61
    );
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(this._ui("Silah Cikar", "Unequip"), unRect.x + unRect.w / 2, unRect.y + unRect.h / 2);
    this.hit.push({ type: "unequip", rect: unRect });

    const rowGap = 10;
    const rowH = isTiny ? 104 : 112;
    ...
    for (const item of WEAPONS) {
      ...
      fillRoundRect(ctx, artRect.x, artRect.y, artRect.w, artRect.h, 14);
      ctx.strokeStyle = active ? "rgba(255,194,96,0.22)" : "rgba(255,255,255,0.08)";
      strokeRoundRect(ctx, artRect.x + 0.5, artRect.y + 0.5, artRect.w - 1, artRect.h - 1, 14);

      const sprite = this._getWeaponSprite(item);
      if (sprite && (sprite.complete || sprite.naturalWidth)) {
        containImage(ctx, sprite, artRect.x + 8, artRect.y + 8, artRect.w - 16, artRect.h - 16, 1);
      } else {
        ctx.fillStyle = "rgba(255,255,255,0.78)";
        ctx.font = `800 ${isTiny ? 8 : 9}px system-ui`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(this._weaponTier(item), artRect.x + artRect.w / 2, artRect.y + artRect.h / 2);
      }
      ...
      const textX = artRect.x + artRect.w + 14;
      const textMaxW = btnRect.x - textX - 14;
      ...
      ctx.fillText(item.name, textX, rowRect.y + 34);

      ctx.fillStyle = "rgba(255,255,255,0.74)";
      ctx.font = `${isTiny ? 10 : 11}px system-ui`;
      ctx.fillText(`${this._ui("Sinif", "Tier")}: ${this._weaponTier(item)}`, textX, rowRect.y + 56);
      ctx.fillText(`${this._ui("Guc", "Power")}: +%${item.bonusPct} - ${this._ui("ikon", "icon")} ~${iconMsFromPct(item.bonusPct)}ms`, textX, rowRect.y + 74);
      ctx.fillText(`${this._ui("Fiyat", "Price")}: ${this._num(item.price)} yton - ${this._weaponLabel(item)}`, textX, rowRect.y + 92);
      ...
      ctx.textBaseline = "middle";
      ctx.fillText(
        !owned ? this._ui("Satin Al", "Buy") : active ? this._ui("Secili", "Equipped") : this._ui("Kusan", "Equip"),
    ...
    ctx.fillText(this._ui(`Aktif Etki: 500ms -> ${effectMs}ms (+%${effectPct})`, `Active Effect: 500ms -> ${effectMs}ms (+%${effectPct})`), footerRect.x + 12, footerRect.y + 28);
