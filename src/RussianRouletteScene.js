(function () {
  const DEFAULTS = {
    buyInCoin: 500,
    buyInEnergy: 2,
    winCoin: 1000,
    winXp: 40,
    finalRefundCoin: 500,
    chamberSize: 6,
    livesPerPlayer: 2,
  };

  const BOT_NAMES = ["ShadowWolf", "NightViper"];

  function clamp(n, min, max) {
    return Math.max(min, Math.min(max, n));
  }

  function lerp(a, b, t) {
    return a + (b - a) * t;
  }

  function roundRectPath(ctx, x, y, w, h, r) {
    const rr = Math.max(0, Math.min(r, w * 0.5, h * 0.5));
    ctx.beginPath();
    ctx.moveTo(x + rr, y);
    ctx.arcTo(x + w, y, x + w, y + h, rr);
    ctx.arcTo(x + w, y + h, x, y + h, rr);
    ctx.arcTo(x, y + h, x, y, rr);
    ctx.arcTo(x, y, x + w, y, rr);
    ctx.closePath();
  }

  function fillRoundRect(ctx, x, y, w, h, r, fill) {
    roundRectPath(ctx, x, y, w, h, r);
    ctx.fillStyle = fill;
    ctx.fill();
  }

  function strokeRoundRect(ctx, x, y, w, h, r, stroke, lw) {
    roundRectPath(ctx, x, y, w, h, r);
    ctx.lineWidth = lw;
    ctx.strokeStyle = stroke;
    ctx.stroke();
  }

  function fitCanvasToParent(canvas) {
    const parent = canvas.parentElement;
    if (!parent) return;

    const rect = parent.getBoundingClientRect();
    const dpr = Math.max(1, window.devicePixelRatio || 1);
    const cssW = Math.max(280, Math.round(rect.width || 320));
    const cssH = Math.max(280, Math.round(rect.height || 420));
    const pxW = Math.round(cssW * dpr);
    const pxH = Math.round(cssH * dpr);

    if (canvas.width !== pxW || canvas.height !== pxH) {
      canvas.width = pxW;
      canvas.height = pxH;
      canvas.style.width = cssW + "px";
      canvas.style.height = cssH + "px";
    }

    const ctx = canvas.getContext("2d");
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  }

  function ensureStyle() {
    if (document.getElementById("tc-roulette-style")) return;

    const style = document.createElement("style");
    style.id = "tc-roulette-style";
    style.textContent = `
      .tc-roulette-root {
        position: absolute;
        inset: 0;
        overflow: hidden;
        background:
          radial-gradient(circle at 50% 18%, rgba(255,90,90,0.20), transparent 30%),
          radial-gradient(circle at 50% 120%, rgba(255,190,80,0.14), transparent 36%),
          linear-gradient(180deg, rgba(18,18,24,0.96), rgba(8,8,12,0.98));
      }

      .tc-roulette-canvas {
        position: absolute;
        inset: 0;
        display: block;
        width: 100%;
        height: 100%;
      }

      .tc-roulette-ui {
        position: absolute;
        left: 0;
        right: 0;
        bottom: 12px;
        display: flex;
        justify-content: center;
        pointer-events: none;
      }

      .tc-roulette-trigger {
        pointer-events: auto;
        appearance: none;
        border: 1px solid rgba(255,255,255,0.18);
        background:
          linear-gradient(180deg, rgba(255,90,90,0.96), rgba(155,24,24,0.96));
        color: #fff;
        border-radius: 18px;
        min-width: 180px;
        height: 52px;
        padding: 0 22px;
        font-size: 15px;
        font-weight: 900;
        letter-spacing: 0.08em;
        box-shadow:
          0 12px 34px rgba(0,0,0,0.34),
          0 0 28px rgba(255,70,70,0.20);
        cursor: pointer;
      }

      .tc-roulette-trigger:disabled {
        opacity: 0.45;
        cursor: default;
        box-shadow: none;
      }
    `;
    document.head.appendChild(style);
  }

  function getNum(obj, path, fallback = 0) {
    try {
      const parts = path.split(".");
      let cur = obj;
      for (const p of parts) {
        if (cur == null) return fallback;
        cur = cur[p];
      }
      const n = Number(cur);
      return Number.isFinite(n) ? n : fallback;
    } catch {
      return fallback;
    }
  }

  function setPath(target, path, value) {
    const parts = path.split(".");
    let cur = target;
    for (let i = 0; i < parts.length - 1; i++) {
      const k = parts[i];
      if (!cur[k] || typeof cur[k] !== "object") cur[k] = {};
      cur = cur[k];
    }
    cur[parts[parts.length - 1]] = value;
  }

  function updateStoreResources(store, delta) {
    if (!store?.get || !store?.set) return;

    const s = store.get() || {};
    const patch = {};

    const topCoins = getNum(s, "coins", NaN);
    const topEnergy = getNum(s, "energy", NaN);
    const topXp = getNum(s, "xp", NaN);

    const profileCoins = getNum(s, "profile.coins", NaN);
    const profileEnergy = getNum(s, "profile.energy", NaN);
    const profileXp = getNum(s, "profile.xp", NaN);

    if (!Number.isNaN(topCoins)) setPath(patch, "coins", Math.max(0, topCoins + (delta.coins || 0)));
    if (!Number.isNaN(topEnergy)) setPath(patch, "energy", Math.max(0, topEnergy + (delta.energy || 0)));
    if (!Number.isNaN(topXp)) setPath(patch, "xp", Math.max(0, topXp + (delta.xp || 0)));

    if (!Number.isNaN(profileCoins)) setPath(patch, "profile.coins", Math.max(0, profileCoins + (delta.coins || 0)));
    if (!Number.isNaN(profileEnergy)) setPath(patch, "profile.energy", Math.max(0, profileEnergy + (delta.energy || 0)));
    if (!Number.isNaN(profileXp)) setPath(patch, "profile.xp", Math.max(0, profileXp + (delta.xp || 0)));

    const hasSomething = Object.keys(patch).length > 0;
    if (hasSomething) store.set(patch);
  }

  function readWallet(store) {
    const s = store?.get?.() || {};
    const coins = Number.isFinite(getNum(s, "coins", NaN))
      ? getNum(s, "coins", 0)
      : getNum(s, "profile.coins", 0);
    const energy = Number.isFinite(getNum(s, "energy", NaN))
      ? getNum(s, "energy", 0)
      : getNum(s, "profile.energy", 0);
    return { coins, energy };
  }

  class RussianRouletteEngine {
    constructor() {
      this.opts = { ...DEFAULTS };
      this.store = null;

      this.root = null;
      this.canvas = null;
      this.ctx = null;
      this.triggerBtn = null;

      this.statusEl = null;
      this.enemyFillEl = null;
      this.meFillEl = null;
      this.enemyHpTextEl = null;
      this.meHpTextEl = null;
      this.opponentEl = null;

      this.running = false;
      this.awaitingShot = false;
      this.processing = false;
      this.ended = false;
      this.paid = false;
      this.finishPlace = null;
      this.finalAwarded = false;
      this.stage = "idle";
      this.message = "Masaya oturmak için başlat.";
      this.turnIndex = 0;
      this.currentChamber = 0;
      this.bulletIndex = 0;
      this.roundNumber = 1;
      this.pendingTimer = null;
      this.botChainTimer = null;
      this.lastTime = 0;
      this.flash = 0;
      this.shake = 0;
      this.muzzle = 0;
      this.spin = 0;
      this.pulse = 0;
      this.winnerId = null;
      this.eliminationLog = [];
      this._raf = 0;
      this._resizeHandler = () => fitCanvasToParent(this.canvas);

      this.players = [];
    }

    init(opts = {}) {
      ensureStyle();

      this.opts = {
        ...DEFAULTS,
        ...opts,
      };
      this.store = opts.store || null;

      this.statusEl = document.getElementById(opts.statusId || "pvpStatus");
      this.enemyFillEl = document.getElementById(opts.enemyFillId || "enemyFill");
      this.meFillEl = document.getElementById(opts.meFillId || "meFill");
      this.enemyHpTextEl = document.getElementById(opts.enemyHpTextId || "enemyHpText");
      this.meHpTextEl = document.getElementById(opts.meHpTextId || "meHpText");
      this.opponentEl = document.getElementById("pvpOpponent");

      const arena = document.getElementById(opts.arenaId || "arena");
      if (!arena) throw new Error("RussianRouletteScene: arena bulunamadı");

      arena.innerHTML = `
        <div class="tc-roulette-root">
          <canvas class="tc-roulette-canvas"></canvas>
          <div class="tc-roulette-ui">
            <button class="tc-roulette-trigger" type="button">TETİĞİ ÇEK</button>
          </div>
        </div>
      `;

      this.root = arena.querySelector(".tc-roulette-root");
      this.canvas = arena.querySelector(".tc-roulette-canvas");
      this.ctx = this.canvas.getContext("2d");
      this.triggerBtn = arena.querySelector(".tc-roulette-trigger");

      fitCanvasToParent(this.canvas);
      window.removeEventListener("resize", this._resizeHandler);
      window.addEventListener("resize", this._resizeHandler);

      this.triggerBtn.onclick = () => {
        if (!this.running || !this.awaitingShot || this.processing || this.ended) return;
        const actor = this.players[this.turnIndex];
        if (!actor || actor.isBot || !actor.alive) return;
        this.fireCurrent();
      };

      this.reset(true);
      this.startLoop();
    }

    setOpponent(opp) {
      const label = opp?.username || "2 Rakip";
      if (this.opponentEl) this.opponentEl.textContent = label;
    }

    buildPlayers() {
      const s = this.store?.get?.() || {};
      const meName =
        s?.profile?.username ||
        s?.user?.username ||
        s?.username ||
        "Sen";

      this.players = [
        {
          id: "me",
          name: meName,
          isBot: false,
          lives: this.opts.livesPerPlayer,
          alive: true,
          eliminatedAt: null,
          seat: 0,
        },
        {
          id: "bot1",
          name: BOT_NAMES[0],
          isBot: true,
          lives: this.opts.livesPerPlayer,
          alive: true,
          eliminatedAt: null,
          seat: 1,
        },
        {
          id: "bot2",
          name: BOT_NAMES[1],
          isBot: true,
          lives: this.opts.livesPerPlayer,
          alive: true,
          eliminatedAt: null,
          seat: 2,
        },
      ];
    }

    start(autoPaidReset = false) {
      if (!autoPaidReset) {
        this.clearTimers();
      }

      if (!this.paid) {
        const wallet = readWallet(this.store);
        if (wallet.coins < this.opts.buyInCoin || wallet.energy < this.opts.buyInEnergy) {
          this.running = false;
          this.awaitingShot = false;
          this.processing = false;
          this.ended = true;
          this.message = `Yetersiz bakiye • ${this.opts.buyInCoin} coin / ${this.opts.buyInEnergy} enerji gerekli`;
          this.setStatus(this.message);
          this.updateHudBars();
          this.updateTrigger();
          return;
        }

        updateStoreResources(this.store, {
          coins: -this.opts.buyInCoin,
          energy: -this.opts.buyInEnergy,
          xp: 0,
        });
        this.paid = true;
      }

      if (!this.players.length || this.ended) {
        this.reset(true);
        this.paid = true;
      }

      this.running = true;
      this.processing = false;
      this.ended = false;
      this.awaitingShot = true;
      this.stage = this.aliveCount() === 3 ? "triple" : "final";
      this.message = `${this.players[this.turnIndex].name} tetiği çekiyor`;
      this.setStatus(`Russian Roulette • Raund ${this.roundNumber}`);
      this.updateHudBars();
      this.updateTrigger();

      const actor = this.players[this.turnIndex];
      if (actor?.isBot) {
        this.scheduleBotShot();
      }
    }

    stop() {
      this.running = false;
      this.awaitingShot = false;
      this.processing = false;
      this.clearTimers();
      this.setStatus("Russian Roulette • Durduruldu");
      this.message = "Oyun durdu.";
      this.updateTrigger();
    }

    reset(keepLoop = false) {
      this.clearTimers();
      this.running = false;
      this.awaitingShot = false;
      this.processing = false;
      this.ended = false;
      this.paid = false;
      this.finishPlace = null;
      this.finalAwarded = false;
      this.stage = "idle";
      this.message = "Masaya oturmak için başlat.";
      this.turnIndex = 0;
      this.currentChamber = 0;
      this.bulletIndex = this.randomBulletIndex();
      this.roundNumber = 1;
      this.flash = 0;
      this.shake = 0;
      this.muzzle = 0;
      this.spin = 0;
      this.pulse = 0;
      this.winnerId = null;
      this.eliminationLog = [];
      this.buildPlayers();
      this.setStatus("Russian Roulette • Hazır");
      this.updateHudBars();
      this.updateTrigger();

      if (!keepLoop) this.startLoop();
    }

    randomBulletIndex() {
      return Math.floor(Math.random() * this.opts.chamberSize);
    }

    startLoop() {
      if (this._raf) cancelAnimationFrame(this._raf);

      const tick = (ts) => {
        if (!this.canvas || !this.ctx) return;
        fitCanvasToParent(this.canvas);

        const dt = Math.min(0.05, Math.max(0.001, (ts - (this.lastTime || ts)) / 1000));
        this.lastTime = ts;
        this.updateVisuals(dt);
        this.render();
        this._raf = requestAnimationFrame(tick);
      };

      this._raf = requestAnimationFrame(tick);
    }

    updateVisuals(dt) {
      this.flash = Math.max(0, this.flash - dt * 2.6);
      this.shake = Math.max(0, this.shake - dt * 28);
      this.muzzle = Math.max(0, this.muzzle - dt * 3.8);
      this.spin += dt * 0.9;
      this.pulse += dt * 3.2;
    }

    clearTimers() {
      if (this.pendingTimer) {
        clearTimeout(this.pendingTimer);
        this.pendingTimer = null;
      }
      if (this.botChainTimer) {
        clearTimeout(this.botChainTimer);
        this.botChainTimer = null;
      }
    }

    alivePlayers() {
      return this.players.filter((p) => p.alive);
    }

    aliveCount() {
      return this.alivePlayers().length;
    }

    nextAliveIndex(fromIndex) {
      if (!this.players.length) return 0;
      let i = fromIndex;
      for (let step = 0; step < this.players.length; step++) {
        i = (i + 1) % this.players.length;
        if (this.players[i].alive) return i;
      }
      return fromIndex;
    }

    scheduleBotShot() {
      this.clearBotOnly();
      this.botChainTimer = setTimeout(() => {
        if (!this.running || !this.awaitingShot || this.processing || this.ended) return;
        const actor = this.players[this.turnIndex];
        if (!actor || !actor.isBot || !actor.alive) return;
        this.fireCurrent();
      }, 900 + Math.random() * 750);
    }

    clearBotOnly() {
      if (this.botChainTimer) {
        clearTimeout(this.botChainTimer);
        this.botChainTimer = null;
      }
    }

    fireCurrent() {
      if (!this.running || this.processing || this.ended) return;
      const actor = this.players[this.turnIndex];
      if (!actor || !actor.alive) return;

      this.processing = true;
      this.awaitingShot = false;

      const exploded = this.currentChamber === this.bulletIndex;
      this.muzzle = exploded ? 1 : 0.35;
      this.shake = exploded ? 10 : 3;
      this.flash = exploded ? 1 : 0.25;

      if (exploded) {
        actor.lives = Math.max(0, actor.lives - 1);
        this.message = `${actor.name} vuruldu!`;
        if (actor.lives <= 0) {
          actor.alive = false;
          actor.eliminatedAt = Date.now();
          this.eliminationLog.push(actor.id);
        }
      } else {
        this.message = `${actor.name} kurtuldu.`;
      }

      this.updateHudBars();
      this.render();

      this.pendingTimer = setTimeout(() => {
        this.resolveAfterShot(actor, exploded);
      }, exploded ? 1150 : 700);
    }

    resolveAfterShot(actor, exploded) {
      this.pendingTimer = null;

      if (exploded) {
        if (!actor.alive) {
          if (actor.id === "me") {
            this.finishPlace = this.aliveCount() === 2 ? 3 : 2;
          }
        }

        if (this.aliveCount() <= 1) {
          this.endMatch();
          return;
        }

        if (this.aliveCount() === 2 && this.stage !== "final") {
          this.stage = "final";
          this.roundNumber += 1;
          this.currentChamber = 0;
          this.bulletIndex = this.randomBulletIndex();
          this.turnIndex = this.nextAliveIndex(this.turnIndex);
          this.processing = false;
          this.awaitingShot = true;
          this.message = "Final raund başladı. Silah yeniden dolduruldu.";
          this.setStatus(`Russian Roulette • Final Raund ${this.roundNumber}`);
          this.updateHudBars();
          this.updateTrigger();

          if (this.players[this.turnIndex]?.isBot) this.scheduleBotShot();
          return;
        }

        this.currentChamber = 0;
        this.bulletIndex = this.randomBulletIndex();
        this.turnIndex = this.nextAliveIndex(this.turnIndex);
      } else {
        this.currentChamber = (this.currentChamber + 1) % this.opts.chamberSize;
        this.turnIndex = this.nextAliveIndex(this.turnIndex);
      }

      this.processing = false;
      this.awaitingShot = true;
      this.message = `${this.players[this.turnIndex].name} sırası`;
      this.setStatus(`Russian Roulette • Raund ${this.roundNumber}`);
      this.updateHudBars();
      this.updateTrigger();

      if (this.players[this.turnIndex]?.isBot) {
        this.scheduleBotShot();
      }
    }

    endMatch() {
      this.running = false;
      this.awaitingShot = false;
      this.processing = false;
      this.ended = true;
      this.clearTimers();

      const winner = this.alivePlayers()[0] || null;
      this.winnerId = winner?.id || null;

      if (winner?.id === "me") {
        this.finishPlace = 1;
      } else if (this.finishPlace == null) {
        this.finishPlace = 2;
      }

      this.applyRewards();
      this.updateHudBars();
      this.updateTrigger();

      if (winner?.id === "me") {
        this.setStatus("Russian Roulette • Kazandın");
        this.message = `Masa senin. +${this.opts.winCoin} coin / +${this.opts.winXp} XP`;
      } else if (this.finishPlace === 2) {
        this.setStatus("Russian Roulette • İkinci oldun");
        this.message = `${winner?.name || "Rakip"} kazandı. Coin iaden yapıldı.`;
      } else {
        this.setStatus("Russian Roulette • Elendin");
        this.message = `${winner?.name || "Rakip"} masayı topladı.`;
      }
    }

    applyRewards() {
      if (this.finalAwarded) return;
      this.finalAwarded = true;

      if (this.finishPlace === 1) {
        updateStoreResources(this.store, {
          coins: this.opts.winCoin,
          energy: 0,
          xp: this.opts.winXp,
        });
      } else if (this.finishPlace === 2) {
        updateStoreResources(this.store, {
          coins: this.opts.finalRefundCoin,
          energy: 0,
          xp: 0,
        });
      }
    }

    setStatus(text) {
      if (this.statusEl) this.statusEl.textContent = text;
    }

    updateTrigger() {
      if (!this.triggerBtn) return;

      if (this.ended) {
        this.triggerBtn.textContent = "TEKRAR OYNA";
        this.triggerBtn.disabled = false;
        this.triggerBtn.onclick = () => {
          this.reset(true);
          this.start();
        };
        return;
      }

      if (!this.running) {
        this.triggerBtn.textContent = "TETİĞİ ÇEK";
        this.triggerBtn.disabled = true;
        return;
      }

      const actor = this.players[this.turnIndex];
      if (!actor) {
        this.triggerBtn.textContent = "TETİĞİ ÇEK";
        this.triggerBtn.disabled = true;
        return;
      }

      if (actor.isBot) {
        this.triggerBtn.textContent = "RAKİP DÜŞÜNÜYOR";
        this.triggerBtn.disabled = true;
      } else {
        this.triggerBtn.textContent = this.processing ? "ATEŞ EDİLİYOR" : "TETİĞİ ÇEK";
        this.triggerBtn.disabled = !this.awaitingShot || this.processing;
      }
    }

    updateHudBars() {
      const me = this.players.find((p) => p.id === "me");
      const enemies = this.players.filter((p) => p.id !== "me");

      const meLives = me ? me.lives : 0;
      const enemyLives = enemies.reduce((a, b) => a + b.lives, 0);
      const enemyAlive = enemies.filter((p) => p.alive).length;

      const meRatio = clamp(meLives / this.opts.livesPerPlayer, 0, 1);
      const enemyRatio = clamp(enemyLives / (this.opts.livesPerPlayer * 2), 0, 1);

      if (this.meFillEl) this.meFillEl.style.transform = `scaleX(${meRatio})`;
      if (this.enemyFillEl) this.enemyFillEl.style.transform = `scaleX(${enemyRatio})`;

      if (this.meHpTextEl) this.meHpTextEl.textContent = String(meLives);
      if (this.enemyHpTextEl) this.enemyHpTextEl.textContent = String(enemyAlive);

      if (this.opponentEl) {
        if (enemyAlive <= 0) this.opponentEl.textContent = "Yok";
        else if (enemyAlive === 1) this.opponentEl.textContent = "1 Rakip";
        else this.opponentEl.textContent = "2 Rakip";
      }
    }

    render() {
      if (!this.ctx || !this.canvas) return;

      const ctx = this.ctx;
      const w = this.canvas.clientWidth || 320;
      const h = this.canvas.clientHeight || 420;

      ctx.clearRect(0, 0, w, h);

      const shakeX = (Math.random() - 0.5) * this.shake;
      const shakeY = (Math.random() - 0.5) * this.shake;

      ctx.save();
      ctx.translate(shakeX, shakeY);

      this.drawBackground(ctx, w, h);
      this.drawHeader(ctx, w, h);
      this.drawPlayers(ctx, w, h);
      this.drawGun(ctx, w, h);
      this.drawFooter(ctx, w, h);

      if (this.flash > 0) {
        ctx.fillStyle = `rgba(255,245,235,${0.18 * this.flash})`;
        ctx.fillRect(0, 0, w, h);
      }

      ctx.restore();
    }

    drawBackground(ctx, w, h) {
      const g = ctx.createLinearGradient(0, 0, 0, h);
      g.addColorStop(0, "rgba(18,18,22,1)");
      g.addColorStop(0.5, "rgba(10,10,14,1)");
      g.addColorStop(1, "rgba(4,4,6,1)");
      ctx.fillStyle = g;
      ctx.fillRect(0, 0, w, h);

      const glow = ctx.createRadialGradient(w * 0.5, h * 0.42, 10, w * 0.5, h * 0.42, w * 0.42);
      glow.addColorStop(0, "rgba(255,90,90,0.18)");
      glow.addColorStop(0.5, "rgba(255,150,50,0.08)");
      glow.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = glow;
      ctx.fillRect(0, 0, w, h);

      for (let i = 0; i < 18; i++) {
        const x = (i / 17) * w;
        const a = 0.02 + 0.01 * Math.sin(this.spin * 2 + i);
        ctx.strokeStyle = `rgba(255,255,255,${a})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(x, h * 0.17);
        ctx.lineTo(x, h);
        ctx.stroke();
      }
    }

    drawHeader(ctx, w, h) {
      const titleY = 18;
      ctx.textAlign = "center";
      ctx.textBaseline = "top";

      ctx.font = "900 24px system-ui, Arial";
      ctx.fillStyle = "rgba(255,255,255,0.96)";
      ctx.fillText("RUSSIAN ROULETTE", w * 0.5, titleY);

      ctx.font = "700 12px system-ui, Arial";
      ctx.fillStyle = "rgba(255,220,180,0.92)";
      ctx.fillText(`6 PATLAR • 1 MERMİ • RAUND ${this.roundNumber}`, w * 0.5, titleY + 30);
    }

    drawPlayers(ctx, w, h) {
      const alive = this.alivePlayers();
      const me = this.players[0];
      const enemyA = this.players[1];
      const enemyB = this.players[2];

      const topY = h * 0.18;
      const bottomY = h * 0.58;
      const cardW = Math.min(150, w * 0.34);
      const cardH = 104;

      this.drawPlayerCard(ctx, enemyA, w * 0.25 - cardW * 0.5, topY, cardW, cardH, this.turnIndex === 1);
      this.drawPlayerCard(ctx, enemyB, w * 0.75 - cardW * 0.5, topY, cardW, cardH, this.turnIndex === 2);
      this.drawPlayerCard(ctx, me, w * 0.5 - cardW * 0.5, bottomY, cardW, cardH, this.turnIndex === 0);

      if (alive.length === 2) {
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.font = "900 18px system-ui, Arial";
        ctx.fillStyle = "rgba(255,200,90,0.95)";
        ctx.fillText("FINAL RAUND", w * 0.5, h * 0.51);
      }
    }

    drawPlayerCard(ctx, player, x, y, w, h, active) {
      const alive = !!player?.alive;
      const pulse = active ? 0.5 + 0.5 * Math.sin(this.pulse * 2.2) : 0;
      const border = alive
        ? active
          ? `rgba(255,90,90,${0.55 + 0.25 * pulse})`
          : "rgba(255,255,255,0.14)"
        : "rgba(255,255,255,0.08)";

      const fill = alive
        ? active
          ? "rgba(42,10,10,0.92)"
          : "rgba(16,16,22,0.82)"
        : "rgba(18,18,18,0.52)";

      fillRoundRect(ctx, x, y, w, h, 18, fill);
      strokeRoundRect(ctx, x, y, w, h, 18, border, active ? 2 : 1);

      if (active && alive) {
        ctx.shadowBlur = 20;
        ctx.shadowColor = "rgba(255,70,70,0.25)";
        strokeRoundRect(ctx, x, y, w, h, 18, "rgba(255,70,70,0.25)", 1.5);
        ctx.shadowBlur = 0;
      }

      ctx.textAlign = "center";
      ctx.textBaseline = "top";
      ctx.font = "800 15px system-ui, Arial";
      ctx.fillStyle = alive ? "rgba(255,255,255,0.95)" : "rgba(255,255,255,0.45)";
      ctx.fillText(player.name, x + w * 0.5, y + 12);

      const role = player.isBot ? "RAKİP" : "SEN";
      ctx.font = "700 11px system-ui, Arial";
      ctx.fillStyle = player.isBot ? "rgba(255,185,120,0.85)" : "rgba(120,220,255,0.85)";
      ctx.fillText(role, x + w * 0.5, y + 34);

      const skull = player.alive ? "" : "☠ ";
      ctx.font = "900 24px system-ui, Arial";
      ctx.fillStyle = player.alive ? "rgba(255,245,235,0.98)" : "rgba(255,90,90,0.90)";
      ctx.fillText(`${skull}${player.lives}`, x + w * 0.5, y + 52);

      const barX = x + 14;
      const barY = y + h - 18;
      const barW = w - 28;
      const ratio = clamp(player.lives / this.opts.livesPerPlayer, 0, 1);
      fillRoundRect(ctx, barX, barY, barW, 8, 6, "rgba(255,255,255,0.08)");
      fillRoundRect(
        ctx,
        barX,
        barY,
        Math.max(0, barW * ratio),
        8,
        6,
        player.alive ? "rgba(255,90,90,0.95)" : "rgba(255,255,255,0.18)"
      );
    }

    drawGun(ctx, w, h) {
      const cx = w * 0.5;
      const cy = h * 0.48;
      const size = Math.min(w, h) * 0.18;
      const active = this.players[this.turnIndex];

      ctx.save();
      ctx.translate(cx, cy);

      const spin = this.spin * 0.35;
      ctx.rotate(spin);

      ctx.shadowBlur = 24;
      ctx.shadowColor = "rgba(255,90,90,0.18)";
      ctx.fillStyle = "rgba(42,42,50,1)";
      ctx.beginPath();
      ctx.arc(0, 0, size, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;

      ctx.fillStyle = "rgba(24,24,28,1)";
      ctx.beginPath();
      ctx.arc(0, 0, size * 0.64, 0, Math.PI * 2);
      ctx.fill();

      for (let i = 0; i < this.opts.chamberSize; i++) {
        const a = (Math.PI * 2 * i) / this.opts.chamberSize - Math.PI / 2;
        const rx = Math.cos(a) * size * 0.44;
        const ry = Math.sin(a) * size * 0.44;

        const isCurrent = i === this.currentChamber && this.running && !this.ended;
        const tone = isCurrent ? "rgba(255,90,90,0.98)" : "rgba(10,10,12,1)";
        ctx.fillStyle = tone;
        ctx.beginPath();
        ctx.arc(rx, ry, size * 0.16, 0, Math.PI * 2);
        ctx.fill();

        if (isCurrent) {
          ctx.strokeStyle = "rgba(255,220,180,0.85)";
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.arc(rx, ry, size * 0.2, 0, Math.PI * 2);
          ctx.stroke();
        }
      }

      ctx.fillStyle = "rgba(110,110,120,1)";
      ctx.beginPath();
      ctx.arc(0, 0, size * 0.18, 0, Math.PI * 2);
      ctx.fill();

      ctx.restore();

      const gunW = size * 1.85;
      const gunH = size * 0.28;
      const gunX = cx - gunW * 0.16;
      const gunY = cy - gunH * 0.5;

      fillRoundRect(ctx, gunX, gunY, gunW, gunH, gunH * 0.4, "rgba(110,110,122,0.98)");
      fillRoundRect(ctx, gunX + gunW * 0.78, gunY + gunH * 0.14, gunW * 0.28, gunH * 0.72, gunH * 0.3, "rgba(132,132,146,1)");

      ctx.save();
      ctx.translate(gunX + gunW * 0.18, gunY + gunH * 0.96);
      ctx.rotate(0.86);
      fillRoundRect(ctx, 0, 0, size * 0.7, size * 0.24, size * 0.08, "rgba(72,44,28,1)");
      ctx.restore();

      if (this.muzzle > 0) {
        const mx = gunX + gunW + size * 0.1;
        const my = gunY + gunH * 0.5;
        const radius = size * (0.32 + this.muzzle * 0.45);
        const burst = ctx.createRadialGradient(mx, my, 0, mx, my, radius);
        burst.addColorStop(0, `rgba(255,245,220,${0.95 * this.muzzle})`);
        burst.addColorStop(0.45, `rgba(255,150,50,${0.55 * this.muzzle})`);
        burst.addColorStop(1, "rgba(255,120,20,0)");
        ctx.fillStyle = burst;
        ctx.beginPath();
        ctx.arc(mx, my, radius, 0, Math.PI * 2);
        ctx.fill();
      }

      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.font = "700 13px system-ui, Arial";
      ctx.fillStyle = "rgba(255,225,190,0.92)";
      ctx.fillText(active ? `${active.name} sırada` : "", w * 0.5, h * 0.67);
    }

    drawFooter(ctx, w, h) {
      const boxW = Math.min(w - 28, 420);
      const boxH = 72;
      const boxX = (w - boxW) * 0.5;
      const boxY = h - 98;

      fillRoundRect(ctx, boxX, boxY, boxW, boxH, 18, "rgba(8,8,12,0.72)");
      strokeRoundRect(ctx, boxX, boxY, boxW, boxH, 18, "rgba(255,255,255,0.10)", 1);

      ctx.textAlign = "center";
      ctx.textBaseline = "top";
      ctx.font = "800 14px system-ui, Arial";
      ctx.fillStyle = "rgba(255,255,255,0.96)";
      ctx.fillText(this.message, w * 0.5, boxY + 14);

      let rewardLine = `Giriş: ${this.opts.buyInCoin} coin / ${this.opts.buyInEnergy} enerji`;
      if (this.ended && this.finishPlace === 1) {
        rewardLine = `Ödül: +${this.opts.winCoin} coin • +${this.opts.winXp} XP`;
      } else if (this.ended && this.finishPlace === 2) {
        rewardLine = `Ödül: ${this.opts.finalRefundCoin} coin iade`;
      } else if (this.ended && this.finishPlace === 3) {
        rewardLine = "Ödül yok";
      }

      ctx.font = "600 12px system-ui, Arial";
      ctx.fillStyle = "rgba(255,210,160,0.88)";
      ctx.fillText(rewardLine, w * 0.5, boxY + 40);
    }

    destroy() {
      this.clearTimers();
      if (this._raf) {
        cancelAnimationFrame(this._raf);
        this._raf = 0;
      }
      window.removeEventListener("resize", this._resizeHandler);
    }
  }

  const engine = new RussianRouletteEngine();

  window.TonCrimePVP_ROULETTE = {
    init(opts) {
      engine.init(opts);
      this._engine = engine;
    },

    setOpponent(opp) {
      engine.setOpponent(opp);
    },

    start() {
      engine.start();
    },

    stop() {
      engine.stop();
    },

    reset() {
      engine.reset(true);
    },

    destroy() {
      engine.destroy();
    },
  };
})();
