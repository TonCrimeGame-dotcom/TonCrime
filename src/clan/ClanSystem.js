...
function isSeededPublicClanId(value) {
  return typeof value === "string" && value.startsWith("pubclan");
}
...
function ensureRootClan(state) {
  ...
  if (state.clan && isSeededPublicClanId(state.clan.id)) state.clan = null;
...
function ensureClanDirectory(state) {
  ...
  cd.clans = cd.clans.filter((clan) => clan && !isSeededPublicClanId(clan.id));
  ...
  cd.player.inbox = [];
  if (cd.player.pendingRequest && isSeededPublicClanId(cd.player.pendingRequest.clanId)) {
    cd.player.pendingRequest = null;
  }
  return cd;
}
...
function ensureClanShape(clan, fallbackPlayerName = "Player") {
  ...
  if (typeof clan.logo !== "string") clan.logo = "";
...
export class ClanSystem {
  ...
  static tickPending(store) {
    return updateStoreState(store, (state) => {
      ...
      const playerDirectory = state.clanDirectory?.player;
      if (Array.isArray(playerDirectory?.inbox)) {
        const currentTime = now();
        playerDirectory.inbox = playerDirectory.inbox.filter((invite) => !invite?.expiresAt || Number(invite.expiresAt) > currentTime);
      }
      const pending = playerDirectory?.pendingRequest;
      if (pending?.expiresAt && now() > Number(pending.expiresAt || 0)) {
        playerDirectory.pendingRequest = null;
        return state;
      }
      if (!pending || state.clan) return state;
      const clanExists = (state.clanDirectory?.clans || []).some((clan) => clan?.id === pending.clanId && !isSeededPublicClanId(clan.id));
      if (!clanExists) playerDirectory.pendingRequest = null;
      return state;
  ...
  static acceptInboxInvite(store, inviteId) {
    return updateStoreState(store, (state) => {
      ...
      state.clanDirectory.player.pendingRequest = {
        ...
        logo: invite.logo || "",
        requestedAt: now(),
        expiresAt: Number(invite.expiresAt || (now() + 24 * 60 * 60 * 1000)),
        source: "invite",
        status: "pending_review",
      };
