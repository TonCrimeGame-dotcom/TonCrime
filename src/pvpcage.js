...
const DAMAGE = {
 ...
 const ICONS = [
  ...
  const api = {
    ...
    _onlineReady: false,
    _onlineReadyPulse: 0,
    _onlinePeerReady: false,
    _onlineStartTimer: 0,
    _onlineStartSent: false,
    _onlineSeenEvents: null,
    ...
    _clearOnlinePulse() {
      if (this._onlineReadyPulse) clearInterval(this._onlineReadyPulse);
      this._onlineReadyPulse = 0;
    },

    _clearOnlineStartTimer() {
      if (this._onlineStartTimer) clearTimeout(this._onlineStartTimer);
      this._onlineStartTimer = 0;
    },

    async _destroyOnlineChannel() {
      this._clearOnlinePulse();
      this._clearOnlineStartTimer();
      this._onlinePeerReady = false;
      this._onlineStartSent = false;
      this._onlineReady = false;

      const sb = this._getRealtimeClient();
      if (this._onlineChannel && sb?.removeChannel) {
        try {
          await sb.removeChannel(this._onlineChannel);
        } catch (_) {}
      }
    ...
    async _broadcastOnline(event, payload = {}) {
      if (!this._onlineChannel || !this._onlineReady) return false;
      try {
    ...
    _rememberOnlineEvent(eventId) {
      const id = String(eventId || "").trim();
      if (!id) return true;
      if (!this._onlineSeenEvents) this._onlineSeenEvents = new Set();
      if (this._onlineSeenEvents.has(id)) return false;
      this._onlineSeenEvents.add(id);
      if (this._onlineSeenEvents.size > 240) {
        const first = this._onlineSeenEvents.values().next().value;
        if (first) this._onlineSeenEvents.delete(first);
      }
      return true;
    },

    _beginLiveRound() {
      if (!this._els || !this._state || this._running) return;
      ...
      this._state.botNextActionAt = this._isOnlineMatch() ? Number.POSITIVE_INFINITY : now + randInt(340, 680);

      this._setStatus("PvP - Kafes dovusu basladi");
      this._toast(this._isOnlineMatch() ? "Rakip baglandi" : "Basladi");
      this._loop();
    },

    _scheduleOnlineStart(startAtMs) {
      if (!this._state || this._state.finished || this._running) return;
      this._clearOnlineStartTimer();

      const delay = Math.max(0, Number(startAtMs || 0) - Date.now());
      this._setStatus("PvP - Baglanti kuruldu");
      this._toast(delay > 0 ? "Mac basliyor" : "Rakip baglandi");

      this._onlineStartTimer = setTimeout(() => {
        this._onlineStartTimer = 0;
        if (!this._state || this._state.finished || this._running) return;
        this._beginLiveRound();
      }, delay);
    },

    async _sendOnlineStartSync() {
      if (!this._isOnlineMatch() || !this._onlineReady || this._onlineStartSent) return false;
      this._onlineStartSent = true;
      this._clearOnlinePulse();
      const startAt = Date.now() + 900;
      await this._broadcastOnline("start_sync", { startAt });
      this._scheduleOnlineStart(startAt);
      return true;
    },

    async _setupOnlineRoom() {
      if (!this._isOnlineMatch()) return false;
      const sb = this._getRealtimeClient();
      if (!sb?.channel) return false;

      await this._destroyOnlineChannel();

      const roomName = `toncrime-cage-${this._matchCtx.matchId}`;
      this._onlineChannel = sb.channel(roomName, {
        config: { broadcast: { self: false, ack: false } },
      });

      this._onlineChannel
        .on("broadcast", { event: "ready" }, async ({ payload }) => {
          if (String(payload?.side || "") === this._mySideKey()) return;
          this._onlinePeerReady = true;
          if (this._matchCtx?.amIPlayer1) {
            await this._sendOnlineStartSync();
          }
        })
        .on("broadcast", { event: "start_sync" }, ({ payload }) => {
          if (!this._state || this._state.finished) return;
          this._clearOnlinePulse();
          this._scheduleOnlineStart(payload?.startAt);
        })
        .on("broadcast", { event: "effect" }, ({ payload }) => {
          this._handleOnlineEffect(payload);
        })
        .on("broadcast", { event: "forfeit" }, ({ payload }) => {
          if (!this._state || this._state.finished) return;
          if (String(payload?.side || "") === this._mySideKey()) return;
          this._finish(true, "Rakip cikti - hukmen galip", {
        ...
        .on("broadcast", { event: "finish" }, ({ payload }) => {
          ...
          if (!winner) return;
          this._finish(winner === this._mySideKey(), String(payload?.message || ""), {
            reason: payload?.reason || (winner === this._mySideKey() ? "win" : "loss"),
            forfeit: !!payload?.forfeit,
            quitter: payload?.quitter || null,
            broadcast: false,
          });
        });
      ...
      this._onlineStartSent = false;
      this._clearOnlinePulse();
      this._onlineReadyPulse = setInterval(() => {
        this._broadcastOnline("ready", { pulse: true }).catch(() => {});
      }, 650);
      await this._broadcastOnline("ready", { pulse: false });
      return true;
    },

    _handleOnlineEffect(payload) {
      if (!this._state || this._state.finished) return;
      if (String(payload?.side || "") === this._mySideKey()) return;
      if (!this._rememberOnlineEvent(payload?.eventId)) return;

      const amount = Math.max(0, Number(payload?.amount || 0));
      const effect = String(payload?.effect || "");
      const meta = getIconMeta(payload?.iconId);
      const enemyName = this._opponent?.username || "Rakip";

      if (effect === "damage_enemy") {
        this._applyDamage("me", amount, `${enemyName} vurdu -${amount} HP`, meta);
        return;
      }

      if (effect === "self_damage") {
        this._applyDamage("enemy", amount, `${enemyName} hata yapti -${amount} HP`, meta);
        return;
      }

      if (effect === "self_heal") {
        this._healEnemy(amount, `${enemyName} can basti +${amount} HP`, meta);
      }
    },

    async _sendOnlineEffect(effect, amount, iconMeta = null) {
      if (!this._isOnlineMatch() || !this._onlineReady || !this._running || this._state?.finished) return;
      const eventId = `${this._mySideKey()}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
      await this._broadcastOnline("effect", {
        eventId,
        effect,
    ...
    reset() {
      ...
      this._onlinePeerReady = false;
      this._onlineStartSent = false;
      this._onlineSeenEvents = new Set();
    ...
    async start() {
      ...
      if (this._isOnlineMatch()) {
        this._setStatus("PvP - Rakip baglaniyor");
        this._toast("Baglaniyor");
        const ready = await this._setupOnlineRoom();
        if (!ready) {
          this._setStatus("PvP - Baglanti kurulamadi");
          this._toast("Baglanti kurulamadI", 1200);
          return;
        }
        return;
      }
    ...
    async stop(opts = {}) {
      ...
      if (shouldForfeit) {
        await this._broadcastOnline("forfeit", { quitter: this._mySideKey() });
        this._finish(false, "Mactan ciktin - hukmen maglup", {
          reason: "quit",
          forfeit: true,
          quitter: "self",
          broadcast: false,
    ...
    _bindEvents() {
      ...
      const onPageHide = () => {
        ...
        this._broadcastOnline("forfeit", { quitter: this._mySideKey() }).catch(() => {});
        this._finish(false, "Mactan ciktin - hukmen maglup", {
          reason: "quit",
          forfeit: true,
          quitter: "self",
...
_handleTap(x, y) {
  ...
  if (cur.bad) {
    ...
    this._sendOnlineEffect("self_damage", cur.damage, cur);
    return;
  }
  ...
  if (cur.heal) {
    ...
    this._sendOnlineEffect("self_heal", cur.damage, cur);
    return;
  }

  this._applyDamage("enemy", cur.damage, `${cur.label} • -${cur.damage} HP`, cur);
  this._sendOnlineEffect("damage_enemy", cur.damage, cur);
},
    ...
    _finish(win, reason, opts = {}) {
      ...
      const message = String(reason || (win ? "Kazandin" : "Kaybettin"));

      this._setStatus(win ? "PvP • Kazandın" : "PvP • Kaybettin");
      this._toast(win ? "Kazandın!" : "Kaybettin!", 1200);

      if (this._isOnlineMatch() && this._onlineReady && opts?.broadcast !== false) {
        this._broadcastOnline("finish", {
          winner: win ? this._mySideKey() : this._enemySideKey(),
          reason: finalReason,
          message,
      ...
      if (!this._resultReported && typeof this.onMatchFinished === "function") {
