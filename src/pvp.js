(function () {
  ...
  function loadPvpGameScript(srcList) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const tryNext = () => {
        if (i >= list.length) {
          reject(new Error("PvP game script could not be loaded: " + list.join(" | ")));
          return;
        }
  ...
  const PVP_TEXT = {
    tr: {
      ...
      headerGeneral: "Rakip Havuzu: Genel",
      overlayFound: "Rakip bulundu",
      overlaySearching: "Rakip aranıyor",
      overlayStartsIn: "Maç {seconds} sn içinde başlıyor",
      overlayPreparing: "Eşleşme hazırlanıyor",
      overlayScanning: "Oyuncular taranıyor...",
      titleChoose: "PvP • Oyun Seç",
      close: "Kapat",
      chipHub: "PvP merkezi",
      chipNightclub: "Nightclub içi havuz",
      chipCoffeeshop: "Coffeeshop içi havuz",
      chipGeneralDetail: "En temiz komboyu yapan parayı toplar.",
      cardOpen: "Açık",
      cardLocked: "Kilitli",
      ...
      statusLoadFailed: "PvP • Oyun yüklenemedi",
      insufficientEnergy: "Yetersiz enerji • {amount} gerekli",
      card_grid_title: "IQ Arena",
      card_grid_subtitle: "Zeka Çatışması",
      card_grid_desc: "Sıra tabanlı grid PvP. Kombolar ile rakibi indir.",
      card_arena_title: "Kafes Dövüşü",
      card_arena_subtitle: "1v1 Kafes Dövüşü",
      card_arena_desc: "Daha hızlı PvP modu. Kritik saldırılar.",
      card_slotarena_title: "Slot Arena",
      card_slotarena_subtitle: "Slot Tadında PvP",
      card_slotarena_desc: "6x6 tumble slot PvP.",
      card_tournament_title: "Kartel Turnuvası",
      card_tournament_subtitle: "Sezonluk PvP",
      card_tournament_desc: "Sezon puanı ve lig mantığı. Şimdilik yakında.",
    },
    en: {
      domReadyStatus: "PvP • Ready",
      ...
      headerGeneral: "Opponent Pool: General",
      overlayFound: "Opponent found",
      overlaySearching: "Searching for opponent",
      overlayStartsIn: "Match starts in {seconds}s",
      overlayPreparing: "Preparing match",
      overlayScanning: "Scanning players...",
      titleChoose: "PvP • Choose Mode",
      close: "Close",
      chipHub: "PvP hub",
      chipNightclub: "Nightclub pool",
      chipCoffeeshop: "Coffeeshop pool",
      chipGeneralDetail: "The cleanest combo takes the cash.",
      cardOpen: "Open",
      cardLocked: "Locked",
      ...
      statusLoadFailed: "PvP • Game failed to load",
      insufficientEnergy: "Not enough energy • {amount} required",
      card_grid_title: "IQ Arena",
      card_grid_subtitle: "Mind Clash",
      card_grid_desc: "Turn-based grid PvP. Drop your rival with combos.",
      card_arena_title: "Cage Fight",
      card_arena_subtitle: "1v1 Cage Fight",
      card_arena_desc: "A faster PvP mode with critical strikes.",
      card_slotarena_title: "Slot Arena",
      card_slotarena_subtitle: "Slot-Style PvP",
      card_slotarena_desc: "6x6 tumble slot PvP.",
      card_tournament_title: "Cartel Tournament",
      card_tournament_subtitle: "Seasonal PvP",
      card_tournament_desc: "Season points and league progression. Coming soon.",
    },
  };

  function normalizePvpLang(lang) {
    return lang === "en" ? "en" : "tr";
  }

  function pvpText(lang, key, fallback = "") {
    const code = normalizePvpLang(lang);
    return PVP_TEXT?.[code]?.[key] ?? PVP_TEXT?.tr?.[key] ?? fallback ?? key;
  }

  function pvpFormat(lang, key, vars = {}, fallback = "") {
    let text = pvpText(lang, key, fallback);
  ...
  function ensurePvpDom(lang = "tr") {
    ensurePvpShellStyle();
    const t = (key, fallback = "") => pvpText(lang, key, fallback);
    ...
    wrap.innerHTML = `
      <div id="pvpHeader">
        <div id="pvpLeftHead">
          <div id="pvpStatus">${t("domReadyStatus")}</div>
          <div id="pvpOpponentRow">
            <span id="pvpSpinner" class="hidden"></span>
            ${t("opponentLabel")}: <b id="pvpOpponent">—</b>
          </div>
        </div>

        <div class="pvpBtns">
          <button class="pvpBtn" id="pvpStart" type="button">${t("start")}</button>
          <button class="pvpBtn" id="pvpStop" type="button">${t("stop")}</button>
          <button class="pvpBtn" id="pvpReset" type="button">${t("reset")}</button>
        </div>
      </div>
      ...
      <div id="pvpBars">
        <div>
          ...
          <div class="pvpBarLabel"><span>${t("enemy")}</span><span id="enemyHpText">100</span></div>
        </div>
        <div>
          <div class="pvpBar"><div class="pvpFill" id="meFill"></div></div>
          <div class="pvpBarLabel"><span>${t("you")}</span><span id="meHpText">100</span></div>
        </div>
      </div>
  ...
  class PvpScene {
    ...
    _text(key, fallback = "") {
      return pvpText(this._lang(), key, fallback);
    }

    _format(key, vars = {}, fallback = "") {
      return pvpFormat(this._lang(), key, vars, fallback);
    }
    ...
    _cardContent(card) {
      ...
      return {
        ...card,
        title: this._text(`card_${card.id}_title`, card.title || ""),
        subtitle: this._text(`card_${card.id}_subtitle`, card.subtitle || ""),
        desc: this._text(`card_${card.id}_desc`, card.desc || ""),
      };
    }

    _modeLabel(id) {
      const card = this.cards.find((item) => item.id === id) || { id };
      return this._cardContent(card).title || "PvP";
    }
    ...
    _buildFallbackMatchContextFromRpc(payload, userId, modeId) {
      ...
      return {
        ...
        opponentUsername:
          ...
          rpc.opponentUsername ||
          (amIPlayer1
            ? (rpc.player2_username || rpc.player2Username || this._text("opponent"))
            : (rpc.player1_username || rpc.player1Username || this._text("opponent"))),
      };
    }
    ...
    async startMatchmaking(id) {
      ...
      if (currentEnergy < energyCost) {
        try {
          window.dispatchEvent(new CustomEvent("tc:toast", {
            detail: {
              text: this._format("insufficientEnergyMode", {
                amount: energyCost,
                mode: this._modeLabel(id),
              }),
            },
          }));
        } catch (_) {}
    ...
    _headerText() {
      if (this.source === "nightclub") return this._text("headerNightclub");
      if (this.source === "coffeeshop") return this._text("headerCoffeeshop");
      return this._text("headerGeneral");
    }
    ...
    async startGame(id, opponentData = null, matchCtx = null) {
      ...
      if (currentEnergy < economy.energy) {
        ...
        try {
          window.dispatchEvent(new CustomEvent("tc:toast", {
            detail: {
              text: this._format("insufficientEnergy", { amount: economy.energy }),
            },
          }));
        } catch (_) {}
        this._launchingGame = false;
      ...
      try {
        ...
        if (id === "grid") {
          ...
          if (dom.status) dom.status.textContent = this._format("statusStarted", { mode: this._modeLabel("grid") });
        ...
        if (id === "slotarena") {
          ...
          if (dom.status) dom.status.textContent = this._format("statusStarted", { mode: this._modeLabel("slotarena") });
          if (dom.spinner) dom.spinner.classList.add("hidden");
          this._launchingGame = false;
          this._resetMatchmaking();
          return;
        ...
        if (id === "arena") {
          ...
          if (dom.status) dom.status.textContent = this._format("statusStarted", { mode: this._modeLabel("arena") });
          if (dom.spinner) dom.spinner.classList.add("hidden");
          this._launchingGame = false;
          this._resetMatchmaking();
          return;
        ...
        if (dom.status) dom.status.textContent = this._text("statusModeMissing");
        if (dom.spinner) dom.spinner.classList.add("hidden");
      } catch (err) {
        refundMatch();
        ...
        if (status) status.textContent = this._text("statusLoadFailed");
    ...
    renderSearchingOverlay(ctx, panelX, panelY, panelW, panelH) {
      ...
      ctx.fillText(this.matchState === "found" ? this._text("overlayFound") : this._text("overlaySearching"), cx, boxY + 46);
      ...
      if (this.matchState === "found" && this.matchOpponent) {
        ...
        ctx.fillText(this._format("overlayStartsIn", { seconds: Math.max(1, Math.ceil(left / 1000)) }), cx, boxY + 178);
      } else {
        ctx.font = "500 15px system-ui, Arial";
        ctx.fillStyle = "rgba(255,255,255,0.78)";
        ctx.fillText(this._text("overlayPreparing"), cx, boxY + 104);
        ctx.font = "700 14px system-ui, Arial";
        ctx.fillStyle = "rgba(255,255,255,0.86)";
        ctx.fillText(this._text("overlayScanning"), cx, boxY + 132);
      }
    }
    ...
    render(ctx) {
      ...
      ctx.fillText(this._text("titleChoose"), panelX + innerPad, panelY + 16);
      ...
      ctx.fillText(this._text("close"), closeX + closeW / 2, closeY + closeH / 2);

      const chipsY = panelY + 16 + titleSize + 10 + subtitleSize + 14;
      const chipH = clamp(Math.round(panelH * 0.06), 28, 36);

      const chips = [
        { id: "general", label: this._text("chipHub") },
        {
          id: this.source,
          label:
            this.source === "nightclub"
              ? this._text("chipNightclub")
              : this.source === "coffeeshop"
              ? this._text("chipCoffeeshop")
              : this._text("chipGeneralDetail"),
        },
      ];
      ...
      for (let i = 0; i < cards.length; i++) {
        const card = this._cardContent(cards[i]);
